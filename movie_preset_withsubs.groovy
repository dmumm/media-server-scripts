#!/usr/bin/env groovy
/*
 * movie_preset_withsubs.groovy - Filebot format expression for organizing movies with subtitles
 *
 * This script determines folder structure and naming convention for movie files
 * with added support for subtitle tracks and special edition indicators.
 */
...
{allOf
    ...
    // Check for special editions and distributors
    { 
    if (f =~ /(?i)criterion/)
        return "Criterion"
    else if (f =~ /(?i)arrow/)
